...
dimf= size(xf,1); % feature state dimension

% for batch update
z_hat= zeros(dimz*lenidf,1); % predictive observation
z= zeros(dimz*lenidf,1); % sensory observation
A= zeros(dimz*lenidf,2*n+1); % stack of innovation covariance for vehicle uncertainty
wc_s= sqrt(wc);
...
for i=1:lenidf
    ...
    % state augmentation
    x_aug = [xv_ ; xfi];
    %P_aug = [Pv_ zeros(dimv,dimz) ; zeros(dimz,dimv) Pfi]; 
...
R_aug = zeros(dimz*lenidf,dimz*lenidf);
for i=1:lenidf
...
U= X*A'; % cross covariance ('dimv' by 'dimz*lenidf')

% Kalman gain
K = U / S;    

% innovation ('dimz*lenidf' by 1) 
v = z-z_hat;
...
Lt= S; % square matrix of 'dimz*lenidf'
