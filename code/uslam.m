...
for t = 2:timestep
    ...
    % Get observation
    for i=1:NPARTICLES
        particles(i).zf = [DDiff(:,t).']; % start from beginning
    end
...
function p = initialize_particles(np,road,vtx)
vtx = reshape(vtx,2,[]);
for i=1:np
    p(i).w= 1/np; % initial particle weight
    p(i).xv= road(:,1); % initial vehicle pose
    p(i).Pv= 0.01*eye(2); % initial robot covariance that considers a numerical error
    p(i).Kaiy= []; % temporal keeping for a following measurement update
    p(i).xf= vtx; % feature mean states
    for j=1:size(vtx,2), p(i).Pf(:,:,j)= 1*eye(2); end % feature covariances    
    p(i).zf= []; % known feature locations
    p(i).idf= []; % known feature index
    p(i).zn= []; % New feature locations  
    p(i).xvl = []; % last timestep state
    p(i).Pvl = []; % last timestep covariance
end
...
h.vtx_r = plot(vtx_r(1,:),vtx_r(2,:),'+b','linewidth',2)
